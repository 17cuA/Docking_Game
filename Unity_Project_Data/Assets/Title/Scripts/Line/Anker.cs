...
// 2019/06/17 [杉山 雅哉] スケールの値が変更されても、ハンドルの位置が正常な距離を保つように修正
//
// TODO：
//
//──────────────────────────────────────────────────────────────────────────
...
public class Anker : MonoBehaviour
{
	...
	//初期化───────────────────────────────────────────────────────────────────────
	public void Init(float scale,Vector3 nomal)
	{
	...
	public void AdjustNextHandleRange(Anker next)
	{
		...
		//!< 06/17 大きさへの対処を追加
		nextHandle.transform.localPosition = new Vector3(0, 0, Vector3.Distance(AnkerPos, next.AnkerPos) / 3) / next.gameObject.transform.localScale.x;
	}
	/// <summary>
	...
	public void AdjustPrevHandleRange(Anker prev)
	{
		//!< ハンドルの影響値は自分の座標が2、次のアンカー座標が1
		//!< 06/17 大きさへの対処を追加
		prevHandle.transform.localPosition = new Vector3(0, 0, -Vector3.Distance(AnkerPos, prev.AnkerPos) / 3) / prev.gameObject.transform.localScale.x;
	}
	//内部メソッド───────────────────────────────────────────────────────────────
	void Update()
	{
	...
	/// </summary>
	/// <returns></returns>
	void ChangeScaleOnlyParent(Transform parent,float scale)
	{
		//!< 子情報を一時保存
